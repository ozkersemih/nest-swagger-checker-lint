...
const apiInformationRule: TSESLint.RuleModule<MessageIds> = {
    defaultOptions: [],
    meta: {
        type: 'problem',
        messages: {
        ...
        docs:{
            description: 'Rule to check whether endpoint has correct information\'s in ApiOperation decorator',
        ...
        return {
            ClassDeclaration(){
                getErrors([[context.filename, context.sourceCode.text]])[context.filename]?.forEach((error: Error) => {
                    if (error.kind === ErrorKind.ApiInformationError){
                        context.report({
...
export default apiInformationRule;
