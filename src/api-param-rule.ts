...
const apiParamRule: TSESLint.RuleModule<MessageIds> = {
    defaultOptions: [],
    meta: {
        type: 'problem',
        messages: {
        ...
        docs:{
            description: 'Rule to check whether param of endpoint has correct information\'s in ApiParam decorator',
        ...
        return {
            ClassDeclaration(){
                getErrors([[context.filename, context.sourceCode.text]])[context.filename]?.forEach((error: Error) => {
                    if (error.kind === ErrorKind.ApiParamError){
                        context.report({
